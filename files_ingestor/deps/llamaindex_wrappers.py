...
class LlamaIndexWrapper:
    @staticmethod
    def mk_index(collection_name: str | None, vector_store: VectorStorePort, embedding_model: EmbeddingModelPort) -> VectorStoreIndex:
        qdrant_vector_store: QdrantVectorStore = vector_store.get_vector_store(collection_name)
        # storage_context = StorageContext.from_defaults(vector_store=qdrant_vector_store)
        index = VectorStoreIndex.from_vector_store(
            vector_store=qdrant_vector_store, embed_model=embedding_model.get_model(), show_progress=True,
            use_async=False)
        return index

    @staticmethod
    def mk_vector_retriever(collection_name: str, similarity_top_k: int, vector_store: VectorStorePort, embedding_model: EmbeddingModelPort) -> VectorIndexRetriever:
    ...
    def mk_autovector_retrieval_tool(
            ...
            llm: FunctionCallingLLMPort) -> QueryEngineTool:
        ...
        retriever_query_engine = RetrieverQueryEngine(
            retriever=retriever,
            response_synthesizer=get_response_synthesizer(llm=llm.get_model(), verbose=True))

        tool_metadata = ToolMetadata(
            description="Provides information and news about Fortune 100 companies",
            name="fortune100_info_news"
        )
    ...
    @staticmethod
    def create_query_engine(collection_name: str,
                            topk: int,
                            vector_store: VectorStorePort,
                            embedding_model: EmbeddingModelPort,
                            llm: FunctionCallingLLMPort) -> Tuple[RetrieverQueryEngine, VectorIndexRetriever]:
