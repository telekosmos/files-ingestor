...
class TestS3StorageAdapter(unittest.TestCase):
    ...
    def test_list_files_non_recursive(self):
        ...
        self.mock_paginator.paginate.assert_called_once_with(
            Bucket='test-bucket', 
            Prefix='folder/'
        )
    ...
    def test_download_file(self):
        ...
        # Download file
        downloaded_path = self.s3_storage_adapter.download_file(
            's3://test-bucket/path/to/file.pdf', 
            local_path
        )
    ...
    def test_invalid_url_raises_error(self):
        ...
        for url in invalid_urls:
            with self.subTest(url=url):
                ...
                with self.assertRaises(ValueError) as context:
                    self.s3_storage_adapter.download_file(url, '/tmp/test.pdf')
                self.assertIn("Invalid S3 URL", str(context.exception))
    ...
    def test_s3_client_error_handling(self):
        ...
        for error_code, error_message in errors:
            with self.subTest(error=error_code):
                ...
                with self.assertRaises(IOError) as context:
                    self.s3_storage_adapter.download_file(
                        's3://test-bucket/file.pdf',
                        '/tmp/test.pdf'
                    )
                self.assertIn(error_message, str(context.exception))
                self.mock_logger.error.assert_called()
