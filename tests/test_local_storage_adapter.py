...
class TestLocalStorageAdapter(unittest.TestCase):
    ...
    def test_list_files_recursive(self):
        ...
        expected = {f"file://{os.path.join(self.temp_dir, path)}" for path in self.test_files}
        self.assertEqual(set(files), expected)
    ...
    def test_invalid_url_raises_error(self):
        ...
        invalid_urls = ["s3://bucket/path", "https://example.com/file", "/absolute/path", "relative/path"]
        ...
        for url in invalid_urls:
            with self.subTest(url=url):
                ...
                with self.assertRaises(ValueError) as ctx:
                    self.storage.download_file(url, "/tmp/test.pdf")  # noqa: S108
                self.assertIn("Invalid file URL", str(ctx.exception))
